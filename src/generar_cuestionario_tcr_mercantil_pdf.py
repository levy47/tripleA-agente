...
de Primas por Tarjeta de Credito (TCR) de Mercantil Seguros, mismo motor que
generar_cuestionario_pdf.py.

Uso:
    python3 src/generar_cuestionario_tcr_mercantil_pdf.py --salida cuestionario_tcr_mercantil.pdf
"""
import argparse
...
def construir(salida):
    ...
    b = Constructor(c, titulo_banner="Cuestionario para Autorización de Débito (Tarjeta de Crédito)",
                     subtitulo_banner="Triple A Seguros — Mercantil Seguros")
    ...
    b.parrafo("Complete al menos la Póliza 1. Puede agregar hasta 4 pólizas. Frecuencia: mensual, "
              "trimestral, semestral o anual.", size=8)
    b.espacio(4)
    for i in range(1, 5):
        b.campos_fila([f"Póliza {i} — Nombre del asegurado:", "Número de póliza:", "Cantidad de cuotas:"])
        b.campos_fila(["Frecuencia:", "Día de cobro:", "Prima:", "Descuento:"])
        b.espacio(4)
...
def main():
    ...
    parser.add_argument("--salida", default="cuestionario_tcr_mercantil.pdf")
